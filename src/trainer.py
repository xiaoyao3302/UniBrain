...
class Trainer:
    ...
    def train(self, local_rank):
        ...
        for epoch in progress_bar:
            ...
            self.sims_image = 0.
            self.sims_text = 0.
            self.sims_final_image = 0.
            self.sims_final_text = 0.

            self.val_sims_image = 0.
            self.val_sims_text = 0.
            self.val_sims_final_image = 0.
            self.val_sims_final_text = 0.

            self.fwd_percent_correct = 0.
            self.bwd_percent_correct = 0.
            self.fwd_percent_final_correct = 0.
            self.bwd_percent_final_correct = 0.

            self.val_fwd_percent_correct = 0.
            self.val_bwd_percent_correct = 0.
            self.val_fwd_percent_final_correct = 0.
            self.val_bwd_percent_final_correct = 0.
            
            self.loss_clip_image_sum = 0.
            self.loss_clip_text_sum = 0.
            self.loss_mse_image_sum = 0.
            self.loss_mse_text_sum = 0.
            self.loss_clip_final_image_sum = 0.
            self.loss_clip_final_text_sum = 0.
            self.loss_mse_final_image_sum = 0.
            self.loss_mse_final_text_sum = 0.
            self.loss_dis_image_sum = 0.
            self.loss_dis_text_sum = 0.
            self.loss_dis_unified_sum = 0.

            self.val_loss_clip_image_sum = 0.
            self.val_loss_clip_text_sum = 0.
            self.val_loss_mse_image_sum = 0.
            self.val_loss_mse_text_sum = 0.
            self.val_loss_clip_final_image_sum = 0.
            self.val_loss_clip_final_text_sum = 0.
            self.val_loss_mse_final_image_sum = 0.
            self.val_loss_mse_final_text_sum = 0.
            self.val_loss_dis_image_sum = 0.
            self.val_loss_dis_text_sum = 0.
            self.val_loss_dis_unified_sum = 0.
    ...
    def train_step(self, voxel, image, captions, subj_id):
        ...
        results = self.voxel2clip(self.input(voxel, subj_id), self.alpha)

        # ====================================================================================================== #
        # loss
        # ====================================================================================================== #
        # outputs include: pred_image_token, pred_text_token, pred_final_image_token, pred_final_text_token
        # for the unified model, the outputs also include: subj_text_pred, subj_image_pred, subj_unified_pred

        # -------------------------------- #
        # final image clip loss
        clip_image_final_pred = results[0]
        clip_image_final_pred_norm = nn.functional.normalize(clip_image_final_pred.flatten(1), dim=-1)
        clip_image_norm = nn.functional.normalize(clip_image.flatten(1), dim=-1)

        if self.args.w_clip_final_image:
            if self.args.mode_clip_loss == 'SoftClip':
                loss_clip_final_image = utils.soft_clip_loss(
                    clip_image_final_pred_norm,
                    clip_image_norm,
                )
            elif self.args.mode_clip_loss == 'Clip':
                loss_clip_final_image = utils.clip_loss(
                    clip_image_final_pred_norm,
                    clip_image_norm,
                )
            elif self.args.mode_clip_loss == 'DualClip':
                loss_clip_final_image = utils.dual_clip_loss(
                    clip_image_final_pred_norm,
                    clip_image_norm,
                )
            elif self.args.mode_clip_loss == 'cosine':
                loss_clip_final_image = utils.cosine_loss(
                    clip_image_final_pred_norm,
                    clip_image_norm,
                )
            else:
                assert self.args.mode_clip_loss == 'SoftClip', 'Unknown CLIP loss'

            utils.check_loss(loss_clip_final_image, "loss_clip_final_image")
            loss += self.args.w_clip_final_image * loss_clip_final_image
            self.loss_clip_final_image_sum += loss_clip_final_image.item()

        # final image mse loss
        if self.args.w_mse_final_image:
            loss_mse_final_image = nn.MSELoss()(clip_image_final_pred_norm, clip_image_norm)
            utils.check_loss(loss_mse_final_image, "loss_mse_final_image")
            loss += self.args.w_mse_final_image * loss_mse_final_image
            self.loss_mse_final_image_sum += loss_mse_final_image.item()
        
        # -------------------------------- #
        # final text clip loss
        clip_text_final_pred = results[1]
        clip_text_final_pred_norm = nn.functional.normalize(clip_text_final_pred.flatten(1), dim=-1)
        clip_text_norm = nn.functional.normalize(clip_text.flatten(1), dim=-1)

        if self.args.w_clip_final_text:
            if self.args.mode_clip_loss == 'SoftClip':
                loss_clip_final_text = utils.soft_clip_loss(
                    clip_text_final_pred_norm,
                    clip_text_norm,
                )
            elif self.args.mode_clip_loss == 'Clip':
                loss_clip_final_text = utils.clip_loss(
                    clip_text_final_pred_norm,
                    clip_text_norm,
                )
            elif self.args.mode_clip_loss == 'DualClip':
                loss_clip_final_text = utils.dual_clip_loss(
                    clip_text_final_pred_norm,
                    clip_text_norm,
                )
            elif self.args.mode_clip_loss == 'cosine':
                loss_clip_final_text = utils.cosine_loss(
                    clip_text_final_pred_norm,
                    clip_text_norm,
                )
            else:
            ...
            utils.check_loss(loss_clip_final_text, "loss_clip_final_text")
            loss += self.args.w_clip_final_text * loss_clip_final_text
            self.loss_clip_final_text_sum += loss_clip_final_text.item()

        # final text mse loss
        if self.args.w_mse_final_text:
            loss_mse_final_text = nn.MSELoss()(clip_text_final_pred_norm, clip_text_norm)
            utils.check_loss(loss_mse_final_text, "loss_mse_final_text")
            loss += self.args.w_mse_final_text * loss_mse_final_text
            self.loss_mse_final_text_sum += loss_mse_final_text.item()

        # -------------------------------- #
        # image clip loss
        clip_image_pred = results[2]
        clip_image_pred_norm = nn.functional.normalize(clip_image_pred.flatten(1), dim=-1)
        clip_image_norm = nn.functional.normalize(clip_image.flatten(1), dim=-1)

        if self.args.w_clip_image:
            if self.args.mode_clip_loss == 'SoftClip':
                ...
                loss_clip_image = utils.dual_clip_loss(
                    clip_image_pred_norm,
                    clip_image_norm,
                )
            elif self.args.mode_clip_loss == 'cosine':
                loss_clip_image = utils.cosine_loss(
                    clip_image_pred_norm,
                    clip_image_norm,
                )
            else:
        ...
        if self.args.w_clip_text:
            if self.args.mode_clip_loss == 'SoftClip':
                ...
                loss_clip_text = utils.dual_clip_loss(
                    clip_text_pred_norm,
                    clip_text_norm,
                )
            elif self.args.mode_clip_loss == 'cosine':
                loss_clip_text = utils.cosine_loss(
                    clip_text_pred_norm,
                    clip_text_norm,
                )
            else:
            ...
            loss += self.args.w_clip_text * loss_clip_text
            self.loss_clip_text_sum += loss_clip_text.item()

        # text mse loss
        if self.args.w_mse_text:
            loss_mse_text = nn.MSELoss()(clip_text_pred_norm, clip_text_norm)
            utils.check_loss(loss_mse_text, "loss_mse_text")
            loss += self.args.w_mse_text * loss_mse_text
            self.loss_mse_text_sum += loss_mse_text.item()

        # -------------------------------- #
        # unified model
        # encourage the model to cannot distinguish the subject the feature belongs to
        # -------------------------------- #
        if self.args.type_model == 'UniBrain' and self.args.w_dis:
            subj_text_pred = results[4]
            subj_image_pred = results[5]
            subj_unified_pred = results[6]

            num_subj = len(self.args.subj_list)
            num_sample_per_subj = subj_text_pred.shape[0] // num_subj

            subj_label_base = torch.ones(num_sample_per_subj).to(subj_image_pred.device)
            subj_label = []
            for sub in range(num_subj):
                subj_label.append(subj_label_base * sub)
            subj_label = torch.cat(subj_label, dim=0).long()

            loss_dis_text = nn.CrossEntropyLoss()(subj_text_pred, subj_label)
            loss_dis_image = nn.CrossEntropyLoss()(subj_image_pred, subj_label)
            loss_dis_unified = nn.CrossEntropyLoss()(subj_unified_pred, subj_label)

            utils.check_loss(loss_dis_text, "loss_dis_text")
            loss += self.args.w_dis * loss_dis_text
            self.loss_dis_text_sum += loss_dis_text.item()

            utils.check_loss(loss_dis_image, "loss_dis_image")
            loss += self.args.w_dis * loss_dis_image
            self.loss_dis_image_sum += loss_dis_image.item()

            utils.check_loss(loss_dis_unified, "loss_dis_unified")
            loss += self.args.w_dis * loss_dis_unified
            self.loss_dis_unified_sum += loss_dis_unified.item()
        ...
        self.sims_final_image += nn.functional.cosine_similarity(clip_image_norm, clip_image_final_pred_norm).mean().item()
        self.sims_final_text += nn.functional.cosine_similarity(clip_text_norm, clip_text_final_pred_norm).mean().item()
        ...
        self.fwd_percent_final_correct += utils.topk(utils.batchwise_cosine_similarity(clip_image_final_pred_norm, clip_image_norm), labels, k=1)
        self.bwd_percent_final_correct += utils.topk(utils.batchwise_cosine_similarity(clip_image_norm, clip_image_final_pred_norm), labels, k=1)
    ...
    def eval_step(self, voxel, image, captions, subj_id):
        ...
        with torch.no_grad():
            ...
            # assert self.args.clip_mult > 0, "clip_mult is default, must be larger than 0!!!"
            ...
            if self.args.w_clip_final_image:
                if self.args.mode_clip_loss == 'SoftClip':
                    val_loss_clip_final_image = utils.soft_clip_loss(
                        clip_image_final_pred_norm,
                        clip_image_norm,
                    )
                elif self.args.mode_clip_loss == 'Clip':
                    val_loss_clip_final_image = utils.clip_loss(
                        clip_image_final_pred_norm,
                        clip_image_norm,
                    )
                elif self.args.mode_clip_loss == 'DualClip':
                    val_loss_clip_final_image = utils.dual_clip_loss(
                        clip_image_final_pred_norm,
                        clip_image_norm,
                    )
                elif self.args.mode_clip_loss == 'cosine':
                    val_loss_clip_final_image = utils.cosine_loss(
                        clip_image_final_pred_norm,
                        clip_image_norm,
                    )
                else:
                ...
                val_loss = self.args.w_clip_final_image * val_loss_clip_final_image
                self.val_loss_clip_final_image_sum += val_loss_clip_final_image.item()

            # final image mse loss
            if self.args.w_mse_final_image:
                val_loss_mse_final_image = nn.MSELoss()(clip_image_final_pred_norm, clip_image_norm)
                val_loss += self.args.w_mse_final_image * val_loss_mse_final_image
                self.val_loss_mse_final_image_sum += val_loss_mse_final_image.item()
            ...
            if self.args.w_clip_final_text:
                if self.args.mode_clip_loss == 'SoftClip':
                    val_loss_clip_final_text = utils.soft_clip_loss(
                        clip_text_final_pred_norm,
                        clip_text_norm,
                    )
                elif self.args.mode_clip_loss == 'Clip':
                    val_loss_clip_final_text = utils.clip_loss(
                        clip_text_final_pred_norm,
                        clip_text_norm,
                    )
                elif self.args.mode_clip_loss == 'DualClip':
                    val_loss_clip_final_text = utils.dual_clip_loss(
                        clip_text_final_pred_norm,
                        clip_text_norm,
                    )
                elif self.args.mode_clip_loss == 'cosine':
                    val_loss_clip_final_text = utils.cosine_loss(
                        clip_text_final_pred_norm,
                        clip_text_norm,
                    )
                else:
                ...
                val_loss += self.args.w_clip_final_text * val_loss_clip_final_text
                self.val_loss_clip_final_text_sum += val_loss_clip_final_text.item()

            # final text mse loss
            if self.args.w_mse_final_text:
                val_loss_mse_final_text = nn.MSELoss()(clip_text_final_pred_norm, clip_text_norm)
                val_loss += self.args.w_mse_final_text * val_loss_mse_final_text
                self.val_loss_mse_final_text_sum += val_loss_mse_final_text.item()
            ...
            if self.args.w_clip_image:
                if self.args.mode_clip_loss == 'SoftClip':
                    ...
                    val_loss_clip_image = utils.clip_loss(
                        clip_image_pred_norm,
                        clip_image_norm,
                    )
                elif self.args.mode_clip_loss == 'DualClip':
                    val_loss_clip_image = utils.dual_clip_loss(
                        clip_image_pred_norm,
                        clip_image_norm,
                    )
                elif self.args.mode_clip_loss == 'cosine':
                    val_loss_clip_image = utils.cosine_loss(
                        clip_image_pred_norm,
                        clip_image_norm,
                    )
                else:
            ...
            if self.args.w_clip_text:
                if self.args.mode_clip_loss == 'SoftClip':
                    ...
                    val_loss_clip_text = utils.clip_loss(
                        clip_text_pred_norm,
                        clip_text_norm,
                    )
                elif self.args.mode_clip_loss == 'DualClip':
                    val_loss_clip_text = utils.dual_clip_loss(
                        clip_text_pred_norm,
                        clip_text_norm,
                    )
                elif self.args.mode_clip_loss == 'cosine':
                    val_loss_clip_text = utils.cosine_loss(
                        clip_text_pred_norm,
                        clip_text_norm,
                    )
                else:
            ...
            # encourage the model to cannot distinguish the subject the feature belongs to
            # -------------------------------- #
            if self.args.type_model == 'UniBrain' and self.args.w_dis and self.args.subj_list == self.args.subj_test_list:
                subj_text_pred = results[4]
                subj_image_pred = results[5]
                subj_unified_pred = results[6]
                ...
                val_loss_dis_text = nn.CrossEntropyLoss()(subj_text_pred, subj_label)
                val_loss_dis_image = nn.CrossEntropyLoss()(subj_image_pred, subj_label)
                val_loss_dis_unified = nn.CrossEntropyLoss()(subj_unified_pred, subj_label)

                val_loss += self.args.w_dis * val_loss_dis_text
                self.val_loss_dis_text_sum += val_loss_dis_text.item()

                val_loss += self.args.w_dis * val_loss_dis_image
                self.val_loss_dis_image_sum += val_loss_dis_image.item()

                val_loss += self.args.w_dis * val_loss_dis_unified
                self.val_loss_dis_unified_sum += val_loss_dis_unified.item()
            ...
            self.val_sims_final_image += nn.functional.cosine_similarity(clip_image_norm, clip_image_final_pred_norm).mean().item()
            self.val_sims_final_text += nn.functional.cosine_similarity(clip_text_norm, clip_text_final_pred_norm).mean().item()
            ...
            self.val_fwd_percent_final_correct += utils.topk(utils.batchwise_cosine_similarity(clip_image_final_pred_norm, clip_image_norm), labels, k=1)
            self.val_bwd_percent_final_correct += utils.topk(utils.batchwise_cosine_similarity(clip_image_norm, clip_image_final_pred_norm), labels, k=1)
    ...
    def save(self, epoch):
        ...
        # save best model
        current_sim = (self.val_sims_final_image + self.val_sims_final_text) / (self.val_i + 1) if hasattr(self, 'val_i') else 0
    ...
    def log_train(self):
        self.logs = {
            ...
            "train/cosine_sim_final_image": self.sims_final_image / (self.train_i + 1),
            "train/cosine_sim_text": self.sims_text / (self.train_i + 1),
            "train/cosine_sim_final_text": self.sims_final_text / (self.train_i + 1),
            "train/fwd_pct_correct": self.fwd_percent_correct / (self.train_i + 1),
            "train/fwd_pct_final_correct": self.fwd_percent_final_correct / (self.train_i + 1),
            "train/bwd_pct_correct": self.bwd_percent_correct / (self.train_i + 1),
            "train/bwd_pct_final_correct": self.bwd_percent_final_correct / (self.train_i + 1),
            "train/loss_clip_image": self.loss_clip_image_sum / (self.train_i + 1),
            "train/loss_clip_final_image": self.loss_clip_final_image_sum / (self.train_i + 1),
            "train/loss_clip_text": self.loss_clip_text_sum / (self.train_i + 1),
            "train/loss_clip_final_text": self.loss_clip_final_text_sum / (self.train_i + 1),
            "train/loss_mse_image": self.loss_mse_image_sum / (self.train_i + 1),
            "train/loss_mse_final_image": self.loss_mse_final_image_sum / (self.train_i + 1),
            "train/loss_mse_text": self.loss_mse_text_sum / (self.train_i + 1),
            "train/loss_mse_final_text": self.loss_mse_final_text_sum / (self.train_i + 1),
            "train/loss_dis_text": self.loss_dis_text_sum / (self.train_i + 1),
            "train/loss_dis_image": self.loss_dis_image_sum / (self.train_i + 1),
            "train/loss_dis_unified": self.loss_dis_unified_sum / (self.train_i + 1),
        }
    ...
    def log_val(self):
        self.logs.update({
            ...
            "val/cosine_sim_final_image": self.val_sims_final_image / (self.val_i + 1),
            "val/cosine_sim_text": self.val_sims_text / (self.val_i + 1),
            "val/cosine_sim_final_text": self.val_sims_final_text / (self.val_i + 1),
            "val/val_fwd_pct_correct": self.val_fwd_percent_correct / (self.val_i + 1),
            "val/val_fwd_pct_final_correct": self.val_fwd_percent_final_correct / (self.val_i + 1),
            "val/val_bwd_pct_correct": self.val_bwd_percent_correct / (self.val_i + 1),
            "val/val_bwd_pct_final_correct": self.val_bwd_percent_final_correct / (self.val_i + 1),
            "val/loss_clip_image": self.val_loss_clip_image_sum / (self.val_i + 1),
            "val/loss_clip_final_image": self.val_loss_clip_final_image_sum / (self.val_i + 1),
            "val/loss_clip_text": self.val_loss_clip_text_sum / (self.val_i + 1),
            "val/loss_clip_final_text": self.val_loss_clip_final_text_sum / (self.val_i + 1),
            "val/loss_mse_image": self.val_loss_mse_image_sum / (self.val_i + 1),
            "val/loss_mse_final_image": self.val_loss_mse_final_image_sum / (self.val_i + 1),
            "val/loss_mse_text": self.val_loss_mse_text_sum / (self.val_i + 1),
            "val/loss_mse_final_text": self.val_loss_mse_final_text_sum / (self.val_i + 1),
            "val/loss_dis_text": self.val_loss_dis_text_sum / (self.val_i + 1),
            "val/loss_dis_image": self.val_loss_dis_image_sum / (self.val_i + 1),
            "val/loss_dis_unified": self.val_loss_dis_unified_sum / (self.val_i + 1),
        })


class Trainer_UniBrain(Trainer):
    def __init__(self, args, accelerator, voxel2clip, clip_extractor, prompts_list, device) -> None:
    ...
    def prepare_dataloader(self):
        ...
        for subj in self.args.subj_list:
            train_dl, val_dl = data.get_dls(
                ...
                num_keyvoxel=self.args.num_keyvoxel,
                num_neighbor=self.args.num_neighbor,
                length=self.args.length,
                seed=self.args.seed,
                use_fMRI_norm=self.args.use_fMRI_norm,
            )
            self.train_dls.append(train_dl)
            self.val_dls.append(val_dl)

        for subj in self.args.subj_test_list:
            test_path = "{}/webdataset_avg_split/test/subj0{}".format(self.args.data_path, subj)
            test_dl = data.get_dataloader(
                ...
                num_keyvoxel=self.args.num_keyvoxel,
                num_neighbor=self.args.num_neighbor,
                perform_group=self.args.perform_group,
                length=self.args.length,
                use_fMRI_norm=self.args.use_fMRI_norm,
            )
    ...
    def prepare_multi_gpu(self):
        self.voxel2clip, self.optimizer, self.lr_scheduler, _, _ = self.accelerator.prepare(
        self.voxel2clip, self.optimizer, self.lr_scheduler, self.train_dls[0], self.test_dls[0])
        ...
        for i, dls in enumerate(self.test_dls):
            test_dl = dls
            self.test_dls[i] = self.accelerator.prepare(test_dl)

    def train_epoch(self, epoch):
        # train loop
        for train_i, datas in enumerate(zip(*self.train_dls)):
            # check the length of the dataset, self.num_batches
            p = float((train_i + epoch * self.num_batches) / (self.args.num_epochs * self.num_batches))
            self.alpha = 2. / (1. + np.exp(-10 * p)) - 1

            self.train_i = train_i
            if self.args.use_mean4train:
                repeat_index = train_i % 4 # randomly choose the one in the repeated three, and one mean
            else:
            ...
            captions = [prompts[0]['caption'] for prompts in current_prompts_list]
    ...
    def eval_epoch(self, epoch):
        ...
        if self.args.subj_list == self.args.subj_test_list:         # depends on your preference, only for vis, as we test the performance using the last model
            ...
            for val_i, datas in enumerate(zip(*self.test_dls)): 
                self.val_i = val_i
                repeat_index = val_i % 3 # randomly choose the one in the repeated three
